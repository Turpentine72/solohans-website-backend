...
import { assertDishStockAvailable, deductDishStockForOrder, DishStockError } from '../utils/dishStockEngine.js';
...
router.post('/', async (req, res) => {
  try {
    ...
    // enough remaining stock for today. Only affects MenuItems with
    // trackDailyStock === true; everything else is untouched. This is the
    // validation step — the actual deduction happens further below, right
    // when the order is confirmed as successfully placed.
    try {
      await assertDishStockAvailable(resolvedItems);
    } catch (err) {
      if (err instanceof DishStockError) {
        return res.status(400).json({ message: err.message });
      }
      throw err;
    }
    ...
    const orderData = {
      ...
      // ✅ Daily Dish Stock is deducted immediately below, the moment this
      // order is successfully placed — not deferred to admin approval like
      // the old MenuItem-based flow. Marking this true here prevents the
      // legacy "first approval" deduction in PATCH /:id/status from ever
      // double-processing this order's dishes.
      stockDeducted: true,
    };
    ...
    // the order is placed — covers WhatsApp orders, Website/Paystack
    // orders, and any other order that comes through the Cart Sidebar.
    // Never fires on "add to cart" — only here, once the order record
    // itself has actually been created. Race-safe against concurrent
    // orders; never goes negative.
    if (resolvedItems.length) {
      try {
        await deductDishStockForOrder(resolvedItems, { orderId: order._id, performedBy: order.customerEmail });
      } catch (err) {
        // Extremely rare race: stock ran out between validation (above) and
        // this write. The order already exists — don't silently oversell,
        // but don't crash the response either; flag it for admin attention.
        console.error('Dish stock deduction error (order already created):', err.message);
      }
    }
